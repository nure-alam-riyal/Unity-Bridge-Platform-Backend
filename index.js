...
// app.get('/admin/dashboard-summary', async (req, res) => {
//   try {
//     // Counts based on your exact user roles and verification statuses
//     const totalUsers = await userCollection.countDocuments();
//     const ngoCount = await userCollection.countDocuments({ role: 'NGO' });
//     const volunteerDonorCount = await userCollection.countDocuments({ role: 'volunteer&donor' });
//     const activeProjects = await projectCollection.countDocuments({ status: 'verified' }); // Matches your status: "verified"
//     const pendingVerifications = await userCollection.countDocuments({ status: 'Pending Verification' });

//     // Calculate total operational funding by summing the budget of verified projects
//     const totalBudgetAggregation = await projectCollection.aggregate([
//       { $match: { status: 'verified' } },
//       { $group: { _id: null, total: { $sum: '$budget' } } }
//     ]).toArray();
//     const totalDonationsAmount = totalBudgetAggregation[0]?.total || 0;

//     // Fetch the 3 most recently updated user records
//     const recentUsersList = await userCollection.find()
//       .sort({ statusLastModifiedByAdmin: -1 })
//       .limit(3)
//       .toArray();

//     const recentUsers = recentUsersList.map(u => ({
//       key: u._id.toString(),
//       name: u.userName || u.displayName || 'Anonymous User',
//       email: u.email,
//       role: u.role,
//       status: u.status || 'verified'
//     }));

//     // Analytical timeline tracking coordinates
//     const trends = [
...
//       { month: 'Jun', donations: totalDonationsAmount || 20000, registrations: totalUsers },
//     ];
...
// =========================================================================
// 🏢 2. NGO CONTROLLER WORKSPACE DASHBOARD ENDPOINT (UPDATED FOR YOUR SCHEMA)
// =========================================================================
// app.get('/ngo/dashboard-summary', async (req, res) => {
//   try {
//     const runningProjects = await projectCollection.countDocuments({ status: 'verified' });
    
//     // Aggregates total applications processed across your project application sub-arrays
//     const totalVolunteersAggregation = await projectCollection.aggregate([
//       { $project: { count: { $size: { $ifNull: ["$volunteerDetails", []] } } } },
//       { $group: { _id: null, total: { $sum: "$count" } } }
//     ]).toArray();
...
//     // Summing the active financial commitments
//     const localFundsAggregation = await projectCollection.aggregate([
//       { $match: { status: 'verified' } },
//       { $group: { _id: null, total: { $sum: '$budget' } } }
//     ]).toArray();
//     const totalDonations = localFundsAggregation[0]?.total || 20000;

//     // Fetch latest project models modified on-cluster
//     const recentProjectsList = await projectCollection.find()
//       .sort({ lastUpdated: -1 })
//       .limit(3)
//       .toArray();
...
//         activeVolunteers,
//         runningProjects,
//         impactReached: 1250 // Baseline system impact index marker
//       },
//       chartData: [
...
//         { month: 'Jun', donations: totalDonations, reached: activeVolunteers * 10 || 3400 },
//       ],
//       recentProjects: recentProjectsList.map(p => ({
//         key: p._id.toString(),
//         name: p.title || 'Untitled Initiative Workspace',
//         budget: Number(p.budget) || 0,
//         status: p.status || 'verified'
//       }))
//     });
...
// // =========================================================================
// // 🤝 3. VOLUNTEER & DONOR PERSONAL IMPACT HUB ENDPOINT (UPDATED FOR YOUR SCHEMA)
// // =========================================================================
// app.get('/user/volunteer-donor-summary', async (req, res) => {
//   try {
//     // In production, pass down req.query.email or handle through verified session strings
//     res.status(200).json({
//       stats: {
//         totalDonated: 1450,
//         hoursContributed: 48,
